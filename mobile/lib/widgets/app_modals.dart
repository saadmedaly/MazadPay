...
class AppModals {
  ...
  static void showAutoBidModal(BuildContext context, {required String auctionId, required double currentPrice}) {
    ...
    final AuctionApi auctionApi = AuctionApi();
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              Text(
                AppLocalizations.of(context)!.text_393, // Auto-Bid
              ...
              _buildTextField(
                ...
                label: 'Pas d\'incrément (Optionnel)',
                hint: 'Ex: 500',
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    final maxBid = double.tryParse(maxBidController.text);
                    if (maxBid == null || maxBid <= currentPrice) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Le montant doit être supérieur au prix actuel')),
                      );
                      return;
                    }
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text(response.message ?? 'Auto-enchère configurée !')),
                      );
                      Navigator.pop(context);
                    }
                  },
  ...
  static void showReportModal(BuildContext context, {required String auctionId}) {
    bool isDarkMode = Theme.of(context).brightness == Brightness.dark;
        ...
        return StatefulBuilder(
          builder: (context, setModalState) {
            return FutureBuilder(
              future: categoryApi.getReportReasons(),
                ...
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 24),
                      Text(
                        AppLocalizations.of(context)!.text_395, // Report
                        style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
                      ),
                      const SizedBox(height: 24),
                      if (reasons.isEmpty && snapshot.connectionState == ConnectionState.waiting)
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: reasons.map((r) {
                            ...
                            final label = r['label_$locale'] ?? r['label_ar'] ?? r['label'] ?? '';
                            return DropdownMenuItem<String>(
                              value: r['id']?.toString(),
                              child: Text(label),
                            );
                          }).toList(),
                          onChanged: (val) => setModalState(() => selectedReasonId = val),
                        ),
                      const SizedBox(height: 16),
                      _buildTextField(
                        context,
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: selectedReasonId == null ? null : () async {
                            ...
                            if (context.mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text(response.message ?? 'Signalement envoyé')),
                              );
                              Navigator.pop(context);
                            }
                          },
                          ...
                          child: const Text('Signaler', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        ),
                      ),
                    ],
                  ),
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          maxLines: maxLines,
          decoration: _inputDecoration(context, hint, isDarkMode, icon: icon),
        ),
      ],
    );
  }

  static InputDecoration _inputDecoration(BuildContext context, String hint, bool isDarkMode, {IconData? icon}) {
