...
class _HowToBidPageState extends State<HowToBidPage> {
  ...
  final TutorialApi _tutorialApi = TutorialApi();
  ...
  Future<void> _loadTutorials() async {
    final response = await _tutorialApi.getTutorials();
    if (mounted) {
      final isArabic = Localizations.localeOf(context).languageCode == 'ar';
      
      setState(() {
        if (response.success && response.data != null && response.data!.isNotEmpty) {
          _tutorials = response.data!.map<Map<String, String>>((item) {
            return {
              'title': isArabic ? (item['title_ar'] ?? '') : (item['title_fr'] ?? item['title_ar'] ?? ''),
            ...
            Center(
              child: InkWell(
                ...
                child: Container(
                  ...
                  child: Icon(
                    _isInitialized && _videoController != null && _videoController!.value.isPlaying ? Icons.pause : Icons.play_arrow,
                    color: Colors.white, 
                    size: 36
                  ),
                ),
