...
class CacheService {
  ...
  static const String _bannersKey = 'banners_data';
  ...
  static const String _userProfileTimestampKey = 'user_profile_timestamp';
  ...
  static const int _bannersCacheTTL = 3600000; // 1 heure pour les bannières
  static const int _userProfileCacheTTL = 3600000; // 1 heure pour le profil utilisateur
  ...
  Future<void> init() async {
    ...
    _bannersBox = await Hive.openBox(_bannersBoxName);
  ...
  // Cache pour les bannières
  Future<void> cacheBanners(List<Map<String, dynamic>> banners) async {
    await _ensureInitialized();
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    await _bannersBox!.put(_bannersKey, jsonEncode(banners));
    await _bannersBox!.put(_bannersTimestampKey, timestamp);
  }
  
  Future<List<Map<String, dynamic>>?> getCachedBanners() async {
    await _ensureInitialized();
    final cachedData = _bannersBox!.get(_bannersKey);
    final timestamp = _bannersBox!.get(_bannersTimestampKey);
    ...
    final now = DateTime.now().millisecondsSinceEpoch;
    if (now - timestamp > _bannersCacheTTL) {
      return null;
    }
  ...
  Future<bool> isBannersCacheValid() async {
    await _ensureInitialized();
    final timestamp = _bannersBox!.get(_bannersTimestampKey);
    if (timestamp == null) return false;
    
    final now = DateTime.now().millisecondsSinceEpoch;
    return now - timestamp <= _bannersCacheTTL;
  }
  
  // Cache pour le profil utilisateur
  Future<void> cacheUserProfile(Map<String, dynamic> userProfile) async {
    await _ensureInitialized();
    final timestamp = DateTime.now().millisecondsSinceEpoch;
    await _userProfileBox!.put(_userProfileKey, jsonEncode(userProfile));
    await _userProfileBox!.put(_userProfileTimestampKey, timestamp);
  }
  
  Future<Map<String, dynamic>?> getCachedUserProfile() async {
    await _ensureInitialized();
    final cachedData = _userProfileBox!.get(_userProfileKey);
    final timestamp = _userProfileBox!.get(_userProfileTimestampKey);
    ...
    final now = DateTime.now().millisecondsSinceEpoch;
    if (now - timestamp > _userProfileCacheTTL) {
      return null;
    }
  ...
  Future<bool> isUserProfileCacheValid() async {
    await _ensureInitialized();
    final timestamp = _userProfileBox!.get(_userProfileTimestampKey);
    if (timestamp == null) return false;
    
    final now = DateTime.now().millisecondsSinceEpoch;
    return now - timestamp <= _userProfileCacheTTL;
  }
  ...
  Future<void> clearUserProfileCache() async {
    await _ensureInitialized();
    await _userProfileBox!.delete(_userProfileKey);
    await _userProfileBox!.delete(_userProfileTimestampKey);
  }
  
  Future<void> clearBannersCache() async {
    await _ensureInitialized();
    await _bannersBox!.delete(_bannersKey);
    await _bannersBox!.delete(_bannersTimestampKey);
  }
}
