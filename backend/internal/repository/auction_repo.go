...
func (r *auctionRepo) CreateLocation(ctx context.Context, l *models.Location) error {
    query := `INSERT INTO locations (city_name_ar, city_name_fr, area_name_ar, area_name_fr) 
              VALUES ($1, $2, $3, $4) RETURNING id`
    return r.db.QueryRowContext(ctx, query, l.CityNameAr, l.CityNameFr, l.AreaNameAr, l.AreaNameFr).Scan(&l.ID)
}

func (r *auctionRepo) UpdateLocation(ctx context.Context, l *models.Location) error {
    query := `UPDATE locations SET city_name_ar = $1, city_name_fr = $2, area_name_ar = $3, area_name_fr = $4 WHERE id = $5`
    _, err := r.db.ExecContext(ctx, query, l.CityNameAr, l.CityNameFr, l.AreaNameAr, l.AreaNameFr, l.ID)
    return err
}
